...
const dependenciesFilePath = path.resolve(__dirname, 'dependencies/index.js');
const dependenciesTemplate = require('./dependencies/template');
...
module.exports = function() {
    ...
    if (shouldWriteManifest) {
        console.log(
            '=========== Writing new src/manifest.js, .core/dependencies.js ==========='
        );
        const fileContents = `
...
module.exports = {
    ...
    listContexts: () => {
        return ${JSON.stringify(manifestConfig.contexts, null, 2)};
    },
    list: () => {
...
`;
        ...
        const dependenciesFileContents = dependenciesTemplate(
            `const contexts = {\n${Object.entries(manifestConfig.contexts)
                .map(([context, pattern]) => {
                    const { modulePath, filePattern } = pattern;
                    return `        '${context}': require.context('${modulePath}', true, /${filePattern}/),\n`;
                })
                .join('')}` + '    };'
        );

        fs.writeFileSync(dependenciesFilePath, dependenciesFileContents);
    }
};
