...
Reactium.Hook.register('component-bindings', async context => {
...
Reactium.Hook.register('app-bindpoint', context => {
    context.appElement = document.getElementById('router');
    return Promise.resolve();
});

Reactium.Hook.register('plugable-config', context => {
    context.plugableConfig = deps().plugableConfig;
    return Promise.resolve();
});

Reactium.Hook.register('app-redux-provider', context => {
    context.Provider = Provider;
    console.log('Defining Redux Provider.');
    return Promise.resolve();
});

Reactium.Hook.register('app-plugable-provider', context => {
    context.PlugableProvider = PlugableProvider;
    console.log('Defining PlugableProvider.');
    return Promise.resolve();
});

Reactium.Hook.register('app-router', context => {
    context.Router = Router;
    console.log('Defining Router.');
    return Promise.resolve();
});

Reactium.Hook.register('app-ssr-mode', context => {
    context.ssr = window && 'ssr' in window && window.ssr;
    return Promise.resolve();
});

Reactium.Hook.register('app-boot-message', (ssr = false, context) => {
